...
class App_Table_20(Toplevel):
    def __init__(self, master = None):
        super().__init__(master = master)
        self.geometry("1142x655")
        ...
        lst = np.empty((21, 4), dtype=object)
        for i in range(21):
            for j in range(4):
                if j==0 :
                    if i>0:
                ...
                if j==1:
                    if i>0:
                        lst[i,j] = " \"{0}\"; \"{1}\" ".format(iteration(20, 1)[1][i-1, 0], iteration(20, 1)[1][i-1, 1])
                    if i==0:
                        lst[i,j] = title[1]
